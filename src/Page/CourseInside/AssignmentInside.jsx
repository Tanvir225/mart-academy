import { useState } from "react";
import { Link } from "react-router-dom";


const AssignmentInside = ({batchDetails}) => {
   const [isOpen, setIsOpen] = useState(null);
    const handleToggle = (idx) => {
        setIsOpen((prevIdx) => (prevIdx === idx ? null : idx));
    };
    return (
        <div className="cursor-pointer space-y-7 w-full h-96 overflow-y-auto px-2">
            {batchDetails?.map((data) => (
                <div key={data._id}>
                    {
                        data?.batchDetails?.modules.map((module) => (
                            <div key={module.moduleId} onClick={() => handleToggle(module.moduleId)} className="flex items-center w-full">
                                {/* the index div  */}
                                <div className="flex size-10 select-none items-center justify-center rounded-md bg-zinc-700 text-xl font-semibold text-white">
                                    <span>{module.moduleId}</span>
                                </div>

                                <div className="relative h-[2px] w-4 bg-teal-300">
                                    <span className="absolute -left-2 -top-[5px] z-40 h-3 w-3 rounded-full border-2 border-zinc-700 bg-white"></span>
                                    <span className="h-1 w-10 bg-teal-200"></span>
                                    <span className={`absolute -right-2 -top-[5px] z-40 h-3 w-3 rounded-full border-2 ${isOpen === module.moduleId ? 'border-zinc-700 bg-white delay-100' : 'border-transparent'}`}></span>
                                </div>

                                {/* main accordion div  */}
                                <div className=" w-full">
                                    <div className="relative  border-t-[12px] border-teal-400 bg-base-100  p-3 shadow-md">
                                        <span className="absolute right-0 top-0 h-0 w-0 border-b-[40px] border-r-[40px] border-b-transparent border-r-zinc-700"></span>
                                        <h1 className="select-none  text-left text-[12px] md:text-base">Module - {module.moduleId}</h1>
                                    </div>
                                    <div className={`grid overflow-hidden text-slate-600 transition-all duration-300 ease-in-out ${isOpen === module?.moduleId ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                                        <div className="overflow-hidden m-3">
                                            <p className=" text-[12px] md:text-sm text-teal-200">Please join this link.</p>
                                            <Link to={module?.assignment || '/my-courses'} className=" underline block py-2 text-sm text-white">Click To Open Assignment for Module - {module.moduleId}</Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ))
                    }
                </div>
            ))}
        </div>
    );
};

export default AssignmentInside;



